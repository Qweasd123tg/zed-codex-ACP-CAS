...
Перед началом реализации новой user-facing фичи, до кодовых изменений, сначала проверять, насколько она реалистична в текущих условиях форка:
...
3. Если видимый UX рисует сам `Zed` и ACP не дает явного поля/метода для управления этим UI, считать фичу Zed-side до доказательства обратного. В таком случае не добавлять adapter-side runtime-задел как "готовую" фичу; сначала фиксировать ограничение и минимальный патч/контракт, который нужен в `Zed`.
4. В описании работы и в docs фиксировать не только «что можно сделать в форке», но и «что нужно подвинуть снаружи», чтобы не создавать ложное ощущение полной готовности.
5. Не держать в runtime-коде полумертвые ветки только ради теоретического parity, если для текущего `Zed`/ACP они не дают практической пользы.
6. Если фича полезна только при особых внешних условиях, это нужно писать прямо:
...
1. У локальных adapter-side конфигов большой продуктовый потенциал, поэтому их schema должна быть отдельным UX-контрактом, а не случайным dump внутреннего state.
2. Не перезаписывать существующий ручной конфиг при parse/validation ошибке. Лучше остановить session/startup с явной ошибкой и путем к файлу, чем молча откатить пользователя в defaults.
