...
- `Context -> Usage`: добавить третий display-пункт для remaining/free контекста. Важно явно отличать его от used %, например `24% free`, чтобы пользователь не путал остаток с заполнением.
- `Context -> Usage`: оставить короткие display-пункты без лишних слов: braille-only (`⣶`), used percentage-only (`76%`), future free percentage (`24% free`). Подробности должны оставаться в hover/description.
