...
| Session config: `mode`, `permissions`, `model`, `reasoning_effort`, `context_control` | `codex-acp-upstream` + форк | `<= 2026-02-18`, `c0b82cc`; `permissions` и `context_control` локально, selector enrich `2026-03-31`, UX rename `2026-04-24` | `[x]` | `[x]` | У нас это разнесено по `src/thread/session/config/*` и `src/thread/session/settings.rs`; `mode` и `permissions` теперь отдельные selectors, а `context_control` показывает `status`, `ctx %`, `MCP`, `skills`, `plugins`, account limits (`5h`/`wk`) и умеет запускать compaction. `status` в short label держит суммарный `used`, а detail/report раскрывает workspace, account и `used / in / out`; account limits дополнительно дают chat-warning при 75/90/95/100% used. |
| Session config: `mode`, `permissions`, `model`, `reasoning_effort`, `context_control` | `codex-acp-upstream` + форк | `<= 2026-02-18`, `c0b82cc`; `permissions` и `context_control` локально, selector enrich `2026-03-31`, UX rename `2026-04-24` | `[x]` | `[x]` | У нас это разнесено по `src/thread/session/config/*` и `src/thread/session/settings.rs`; `mode` и `permissions` теперь отдельные selectors, а `context_control` показывает `status`, `ctx %`, `MCP`, `skills`, `plugins`, account limits (`5h`/`wk`) и умеет запускать compaction. `status` в short label держит суммарный `used`, а detail/report раскрывает workspace, account и `used / in / out`; account limits дополнительно дают chat-warning при 75/90/95/100% used. |
