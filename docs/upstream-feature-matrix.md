...
| Session config: `mode`, `permissions`, `model`, `reasoning_effort`, `context_control` | `codex-acp-upstream` + форк | `<= 2026-02-18`, `c0b82cc`; `permissions` и `context_control` локально, selector enrich `2026-03-31`, UX rename `2026-04-24`, grouped picker polish `2026-04-28`, model grouping `2026-05-05` | `[x]` | `[x]` | У нас это разнесено по `src/thread/session/config/*` и `src/thread/session/settings.rs`; `mode` и `permissions` остаются отдельными selectors, а `Model` теперь grouped control для `Models` / `Reasoning` / `Speed`, чтобы нижняя панель не раздувалась отдельными `Reasoning Effort` и `Speed`. Так как ACP select передает только один `current_value`, активные nested пункты `Reasoning`/`Speed` помечаются `★` прямо в коротком option label, а подробная сводка model/reasoning/speed уходит в description текущей model option. `context_control` в нижней панели показывает только короткое значение (`%`, `---`, `Compacting...`), а внутри selector держит session status, `MCP`, `skills`, `plugins`, account limits (`5h`/`wk`) и compaction. В Zed config selector descriptions сейчас plain text, не Markdown, поэтому визуальный polish идет через короткие option labels и ACP grouped select options: `Context` группируется в `Usage` / `Integrations` / `Limits` / `Actions`, `Permissions` — в guarded/bypass. Account limits дополнительно дают компактный emoji-notice при 75/90/95/100% used, чтобы системный текст не сливался с ответом агента. |
