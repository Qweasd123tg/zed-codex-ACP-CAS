...
- Нижние selectors `Context` и `Permissions` используют тот же session/config lifecycle. В текущем `Zed` descriptions у config options
  ...
  `Model` делит пункты на `Models`, `Reasoning`, `Speed`, `Context` — на `Display`, `Context`, `Limits`, `Integrations`, `Actions`, а `Permissions` — на workflow, guarded и bypass режимы.
...
- Read-only пункты `Context` (`Status`, `MCP`, `Skills`, `Plugins`) не отправляют summary в чат при клике: Zed уже показывает подробности в hover/description. Группа `Display` выбирает, что показывает нижняя кнопка (`context`, `limits`, `context+limits`), а группы `Context` и `Limits` ниже только переключают стиль отображения и не становятся selected mode; явный chat-report остается за `/status`.
...
- В группе `Context -> Display` есть три режима нижней кнопки: context-only, limits-only и combined context+limits. В группе `Context` есть два компактных style-пункта для context: braille-only (`⠀` / `⣤` / `⣿`) и percentage-only (`---` / `50%` / `100%`); сами style-пункты показывают пример текущего значения (`Braille ⣶`, `Percent 76%`) и в hover описывают стиль, а не дублируют весь status. В группе `Limits` есть text style (`5h 80%`), five-cell remaining style (`▰▰▰▰▱`, где `▰▰▰▰▰` = 100% остатка и `▱▱▱▱▱` = 0%) и single-block remaining style (`▁`..`█`). Сам hover у выбранного display mode при этом обязан оставаться человекочитаемым: процент, exact tokens, status (`live` / `cached` / `pending` / `compacting`) и account limits отдельными строками.
- Вложенные style-настройки selectors дополнительно сохраняются adapter-side в `$CODEX_HOME/memories/codex-acp/selector-preferences.json`, потому что ACP/Zed отдает только один `currentValue` на selector и не может выразить одновременно `context+limits` + `Braille` + `Block`. Persistence best-effort: ошибка записи не должна ломать сам выбор пункта.
