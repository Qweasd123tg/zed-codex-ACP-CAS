...
- Старые context usage display modes удалены после появления нативного Zed context donut для external ACP agents, но нижний `Status` selector сохранен: `Limits`, `Session`, `MCP`, `Skills`, `Plugins`, `Compact now`.
...
- Hover/description у самих selectors собирается из текущего runtime state: `Permissions` показывает active workflow и сохраненный sandbox/approval профиль, `Model` — текущие model/reasoning/speed, `Status` — полный status report.
- Account limit labels идут через `~/.codex-cas/display-maps.json` как `percent -> label` maps; `limits.summary` управляет только короткой строкой `Limits` ordered-list форматом (`["primary"]` или `["primary", "secondary"]`), а hover/status details продолжают показывать полный лимитный контекст. Model labels берутся из `model_selector.models[*].name`, effort labels/descriptions — из `model_selector.reasoning_efforts[*].name` / `description`; если override не задан, используются backend/code defaults. Отдельных model-prefix, effort-icon, speed-glyph и context-display style layers больше нет.
...
- Account rate limits дополнительно дают одноразовые chat-advisory при переходе через 75/90/95/100% использованного окна; состояние порогов хранится в `ThreadInner`,
  а форматирование находится в `src/thread/session/config/limits.rs`, чтобы `/status` и warning-текст не расходились.
...
- После обычного model turn адаптер best-effort обновляет account rate limits через `account/rateLimits/read` с коротким timeout и тем же `AccountRateLimitsUpdated` path, чтобы `/status` и warning state не ждали следующего prompt для перехода, например, с `100%` на фактический `70%`.
- `ThreadTokenUsageUpdated` остается adapter-side forwarding в ACP `UsageUpdate`. После обновления ACP/Zed `src/thread/turn/notify.rs` публикует `usage_update` из cached context usage на `load`/`resume` и из live `ThreadTokenUsageUpdated` после turn, а текущий Zed уже рисует нативный context donut для external ACP agent без adapter-side context-percent/braille display modes.
