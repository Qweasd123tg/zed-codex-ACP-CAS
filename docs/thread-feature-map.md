...
- Нижние selectors `Context` и `Permissions` используют тот же session/config lifecycle. В текущем `Zed` descriptions у config options
  ...
  `Model` делит пункты на `Models`, `Effort`, `Speed`, `Context` — на `Display`, `Integrations`, `Actions`, а `Permissions` — на workflow, guarded и bypass режимы.
  У ACP select есть только один `current_value`, поэтому выбранные nested пункты `Reasoning`/`Speed` помечаются adapter-side через `★` в option label; в `Context` selector текущим значением остается только display mode.
...
- `model_selector` в `$CODEX_HOME/codex-acp/selector-preferences.json` задает `default_model`, `default_reasoning_effort`, `default_service_tier`, `models` и `reasoning_efforts`. `models` / `reasoning_efforts` — boolean-таблицы видимости в стиле `slash_commands`: `true` показывает пункт, `false` скрывает. Выбор model/effort/speed через selector теперь сохраняется как default; фильтры прячут пункты только из UI и всегда оставляют текущую model/effort видимой, чтобы ACP `currentValue` не указывал на отсутствующий option. `none` / `minimal` могут быть явно включены как protocol/experimental effort-пункты даже если текущий backend model list их не рекламирует; backend все еще может отклонить неподдержанную комбинацию. Старые `hidden_models` / `hidden_reasoning_efforts` читаются как legacy input и при следующем сохранении нормализуются в таблицы.
...
- Read-only пункты `Context` (`Status`, `MCP`, `Skills`, `Plugins`) не отправляют summary в чат при клике: Zed уже показывает подробности в hover/description. Группа `Display` выбирает, что показывает нижняя кнопка (`context`, `limits`, `context+limits`); visual styles задаются в adapter config, а явный chat-report остается за `/status`.
- `/status` использует ту же компактную структуру, что hover нижнего selector: `Context`, `Limits`, `Session`, `Integrations`, поэтому чатовый отчет и нижняя панель не расходятся по смысловым блокам.
...
- В группе `Context -> Display` есть три режима нижней кнопки: context-only, limits-only и combined context+limits. Стиль context (`percent`/`braille`), стиль limits (`text`/`bars`/`block`), model label style и effort display style больше не шумят в selector UI и задаются через `$CODEX_HOME/codex-acp/selector-preferences.json`. Сам hover у выбранного display mode при этом обязан оставаться человекочитаемым: процент, exact tokens, status (`live` / `cached` / `pending` / `compacting`) и account limits отдельными строками.
- Вложенные style/layout-настройки selectors дополнительно сохраняются adapter-side в `$CODEX_HOME/codex-acp/selector-preferences.json` (legacy fallback: `$CODEX_HOME/memories/codex-acp/selector-preferences.json`), потому что ACP/Zed отдает только один `currentValue` на selector и не может выразить одновременно `context+limits` + `Braille` + `Block`. Layout config ограниченно управляет только известными selectors: `order`, `visible`, `name`, `groups`; произвольные option values не поддерживаются. Persistence best-effort: ошибка записи не должна ломать сам выбор пункта.
- Существующий `selector-preferences.json` не перезаписывается на старте только ради materialize, чтобы почти правильный ручной конфиг не сбрасывался в defaults. Новый файл создается без inert/example секций: сам default config уже является редактируемым примером.
...
Turn-level backend errors могут прийти дважды: сначала как `ServerNotification::Error`, затем как `TurnCompleted(Failed)` с тем же текстом. `ThreadInner.last_turn_error_notice` дедуплицирует одинаковый error text внутри одного turn, чтобы пользователь не видел пару `Turn error` / `Turn failed` с одинаковым usage-limit сообщением.
...
Для context compaction отдельная инварианта: `/compact` и selector `Compact now` не должны держать `ThreadInner` во время `thread/compact/start`. Сначала нужно локально выставить `compaction_in_progress`, отправить системный status и config update, а backend start ждать уже без inner-lock и с bounded app-server timeout. Иначе backend delay выглядит как немой hang до первого UI-сигнала.
