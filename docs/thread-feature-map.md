...
- Hover/description у самих selectors тоже собирается из текущего runtime state: `Mode` показывает активный режим, `Permissions` — активный sandbox/approval профиль, `Model` — текущие model/reasoning/speed, а `Context` — выбранный context/limits summary без необходимости открывать пункт списка.
