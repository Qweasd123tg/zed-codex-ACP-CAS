...
- `~/.codex-cas/selector-preferences.json` теперь читается как JSONC: `//` и `/* ... */` комментарии допустимы, trailing commas тоже поддерживаются. Существующий битый файл считается ошибкой старта session, а не поводом откатиться к defaults или переписать ручной конфиг. Новый generated формат разложен на секции `display`, `defaults`, `model_selector`, `layout`, `slash_commands`. `CODEX_CAS_HOME` может переопределить adapter-owned directory; legacy files из `$CODEX_HOME/codex-acp/` и `$CODEX_HOME/memories/codex-acp/` копируются в новый дом только если новых файлов еще нет. `defaults` задает `model`, `reasoning_effort` и `service_tier`; выбор model/effort/speed через selector сохраняется как default. Compact context label и account limit labels вынесены в отдельный `~/.codex-cas/display-maps.json`; готовые примеры прежних context `percent` / `braille` и limit `text` / `bars` / `block` вариантов лежат в `examples/display-maps/` и проверяются тестом loader-а.
