//! Доменные правила file-change потока:
//! пути, превью/реплей диффов и логика подтверждений правок.
...
pub(in crate::thread) fn file_change_to_preview_diff(
    ...
    match &change.kind {
        ...
        PatchChangeKind::Update { move_path } => {
            ...
            // Предпочитаем точное восстановление из захваченного pre-edit контента.
            ...
            // Fallback, когда старый snapshot недоступен/несовместим (например, восстановленная история):
            // выводим обе стороны напрямую из hunk unified diff.
...
pub(in crate::thread) fn file_change_to_replay_diff(
    ...
    match change.kind {
        PatchChangeKind::Add => {
            // Некоторые исторические события могут нести add/delete как unified hunk вместо сырого контента.
            // Когда возможно, предпочитаем распарсенные old/new, чтобы replay-диффы оставались богато подсвеченными.
