//! Минимальный парсер unified diff для восстановления снапшотов текста до/после для превью.
...
// Сначала парсим диапазоны @@, потому что от них зависит вся логика применения hunk.
...
fn parse_unified_diff_hunks(unified_diff: &str) -> Option<Vec<UnifiedDiffHunk>> {
    // Парсим только нужное подмножество unified diff: заголовки hunk и
    // тела со строковыми префиксами (' ', '+', '-'). Заголовки файлов и суффиксы move игнорируются.
...
pub(super) fn apply_unified_diff_to_text(old_text: &str, unified_diff: &str) -> Option<String> {
    // Best-effort восстановление текста после правки из unified diff.
    // Проверяем context/deleted строки относительно old_text, чтобы не собрать неверное превью.
    let hunks = parse_unified_diff_hunks(unified_diff)?;
