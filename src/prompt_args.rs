//! Prompt argument parser for slash commands and mention expansion.

/// Mostly copied from `codex_tui::bottom_pane::prompt_args`:
/// <https://github.com/zed-industries/codex/blob/9baf30493dd9f531af1e4dc49a781654b1b2c966/codex-rs/tui/src/bottom_pane/prompt_args.rs#L1>
...
impl PromptArgsError {
    // Keep the parsed command shape explicit so callers can surface clear errors.
...
/// Parses a first-line slash command of the form `/name <rest>`.
/// Returns `(name, rest_after_name)` if the line starts with `/` and contains
/// a non-empty command name; otherwise returns `None`.
...
///
/// A placeholder is any token matching `$[A-Z][A-Z0-9_]*`, for example `$USER`.
/// Returns placeholder names without the leading `$`, without duplicates,
...
pub fn prompt_argument_names(content: &str) -> Vec<String> {
    ...
    for m in PROMPT_ARG_REGEX.find_iter(content) {
        ...
        // Exclude the special positional aggregation token from named arguments.
        if name == "ARGUMENTS" {
            continue;
        }
...
/// Parses `key=value` pairs that follow a custom prompt name.
///
/// Input is split using shlex rules, so quoted values are supported
/// (for example, `USER="Alice Smith"`). Returns a map of parsed arguments,
/// or an error if a token has no `=` or an empty key.
...
/// Expands a message of the form `/prompts:name [value] [value] …` using the matching saved prompt.
///
/// If the text does not start with `/prompts:` or the prompt named `name` is missing,
/// returns `Ok(None)`. On success it returns `Ok(Some(expanded))`;
/// otherwise it returns a user-readable error.
pub fn expand_custom_prompt(
    ...
    // If named placeholders exist, require `key=value` input.
    ...
    // Otherwise treat it as a prompt with positional placeholders, or none.
    let pos_args: Vec<String> = Shlex::new(rest).collect();
    let expanded = expand_numeric_placeholders(&prompt.content, &pos_args);
    Ok(Some(expanded))
}

/// Expands `$1..$9` and `$ARGUMENTS` inside `content` using values from `args`.
